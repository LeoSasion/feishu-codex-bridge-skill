...
BRIDGE_VERSION = "4.2.0-alpha.30"

BOOLEAN_ENV_DEFAULTS = {
    "CODEX_BRIDGE_ALLOW_PROJECT_CREATE": False,
...
ENUM_ENV_SPECS = {
    ...
    "CODEX_BRIDGE_REPLY_FORMAT": ("markdown", frozenset({"text", "markdown"})),
}
INTEGER_ENV_SPECS: dict[str, tuple[int, int, int | None]] = {
    "CODEX_BRIDGE_EVENT_READY_TIMEOUT": (15, 5, 120),
    "CODEX_BRIDGE_ROUTER_TIMEOUT": (3600, 30, 86400),
    "CODEX_BRIDGE_ROUTER_HEARTBEAT_TTL": (90, 15, 3600),
    "CODEX_BRIDGE_ROUTER_CLAIM_TTL": (7200, 60, 86400),
    "CODEX_BRIDGE_ROUTER_RETENTION_HOURS": (168, 1, 8760),
    "CODEX_BRIDGE_ROUTER_WAKE_TTL": (180, 60, 900),
    "CODEX_BRIDGE_GATEWAY_SCHEDULER_TTL": (300, 120, 3600),
    "CODEX_BRIDGE_ROUTER_GRACE_MAX_SECONDS": (30, 0, 60),
...
class BridgeConfig:
    ...
    router_heartbeat_ttl_seconds: int
    router_claim_ttl_seconds: int
    router_retention_hours: int
    router_wake_ttl_seconds: int
    gateway_scheduler_ttl_seconds: int
    router_grace_max_seconds: int
    max_reply_chars: int
...
def load_config() -> BridgeConfig:
    ...
    projects_value = os.environ.get("CODEX_BRIDGE_PROJECTS_ROOT", "").strip()
    projects_root = (
        Path(projects_value).resolve()
        if projects_value
    ...
    return BridgeConfig(
        ...
        allow_project_create=_bool_env("CODEX_BRIDGE_ALLOW_PROJECT_CREATE"),
        ...
        router_timeout_seconds=_int_env("CODEX_BRIDGE_ROUTER_TIMEOUT"),
        router_heartbeat_ttl_seconds=_int_env("CODEX_BRIDGE_ROUTER_HEARTBEAT_TTL"),
        router_claim_ttl_seconds=_int_env("CODEX_BRIDGE_ROUTER_CLAIM_TTL"),
        router_retention_hours=_int_env("CODEX_BRIDGE_ROUTER_RETENTION_HOURS"),
        router_wake_ttl_seconds=_int_env("CODEX_BRIDGE_ROUTER_WAKE_TTL"),
        gateway_scheduler_ttl_seconds=_int_env("CODEX_BRIDGE_GATEWAY_SCHEDULER_TTL"),
        router_grace_max_seconds=_int_env("CODEX_BRIDGE_ROUTER_GRACE_MAX_SECONDS"),
