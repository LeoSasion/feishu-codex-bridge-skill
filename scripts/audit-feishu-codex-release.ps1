...
function Test-SkillMetadata {
    ...
    $text = $script:StrictUtf8.GetString([byte[]]$BytesByPath['SKILL.md'])
    $lines = [regex]::Split($text, "\r\n|\n|\r")
    if ($lines.Count -lt 4 -or $lines[0] -cne '---') {
        Add-AuditFinding -RuleId 'INVALID_SKILL_FRONTMATTER' -Component $Component -Path 'SKILL.md'
        return
    }
    $closing = -1
    ...
    $interfaceText = $script:StrictUtf8.GetString([byte[]]$BytesByPath['agents/openai.yaml'])
    foreach ($marker in @('display_name:', 'short_description:', 'default_prompt:')) {
        if ($interfaceText -notmatch [regex]::Escape($marker)) {
            Add-AuditFinding -RuleId 'INVALID_SKILL_INTERFACE' -Component $Component -Path 'agents/openai.yaml' -Candidate $marker
        }
    }
}
...
if ([string]$inventory.source_version -ne '4.2.0-alpha.30') { throw 'Release inventory source version is not current.' }
...
$workspaceAgents = Join-Path $desktop 'AGENTS.md'
...
if (-not $DesktopOnly) {
    ...
    $handoff = $script:StrictUtf8.GetString(
        [byte[]]$firstBytesByComponent['desktop_bridge']['HANDOFF.md']
    )
    foreach ($frozen in @(Get-PropertyArray -Object $harnessComponent -Name 'frozen_sha256')) {
        ...
        foreach ($binding in @(
            [pscustomobject]@{ Text = $harnessReference; Path = 'references/external-lab.md'; Component = 'harness_sibling' },
            [pscustomobject]@{ Text = $handoff; Path = 'HANDOFF.md'; Component = 'desktop_bridge' }
        )) {
            if ([string]$binding.Text -notmatch [regex]::Escape($expectedHash)) {
                Add-AuditFinding -RuleId 'HARNESS_FROZEN_HASH_UNBOUND' -Component ([string]$binding.Component) `
                    -Path ([string]$binding.Path) -Candidate $relative
            }
        }
    }
}
