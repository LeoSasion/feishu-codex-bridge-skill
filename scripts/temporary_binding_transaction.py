"""Create or roll back one owner-approved diagnostic Feishu binding.
...
read-only task tools.  Mutations require a stopped Listener and are protected by
the same bridge lock filename used by the Listener.
"""
...
class MaintenanceLock:
    """Fail closed unless the normal Listener has released its runtime lock."""
    ...
    def __enter__(self) -> "MaintenanceLock":
        ...
        if health_status and health_status != "stopped":
            raise ValueError("Listener health is not stopped")
        if self.pid_path.exists():
            try:
                pid = int(self.pid_path.read_text(encoding="utf-8").strip())
            except (OSError, ValueError):
                raise ValueError("Listener pid reference is present but invalid") from None
            if _process_exists(pid):
                raise ValueError("Listener pid is still active")
            raise ValueError("stale Listener pid reference must be repaired before maintenance")
        try:
            descriptor = os.open(self.lock_path, os.O_CREAT | os.O_EXCL | os.O_WRONLY)
        except FileExistsError:
            raise ValueError("Listener or another maintenance action still owns the bridge lock") from None
        try:
...
def bind(args: argparse.Namespace) -> dict[str, Any]:
    if not args.listener_stopped_acknowledged:
        raise ValueError("binding requires an explicit stopped-Listener acknowledgement")
    thread_id = args.thread_id.strip()
    if not THREAD_ID_PATTERN.fullmatch(thread_id):
        raise ValueError("target thread id has an unsupported format")
...
def rollback(args: argparse.Namespace) -> dict[str, Any]:
    if not args.listener_stopped_acknowledged:
        raise ValueError("rollback requires an explicit stopped-Listener acknowledgement")
    runtime_dir = _runtime_dir(args.runtime_dir)
    with MaintenanceLock(runtime_dir):
        store = SessionStore(runtime_dir / "sessions.json")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    bind_parser.add_argument("--listener-stopped-acknowledged", action="store_true")

    rollback_parser = subcommands.add_parser("rollback")
    rollback_parser.add_argument("--transaction-id", required=True)
    rollback_parser.add_argument("--listener-stopped-acknowledged", action="store_true")
