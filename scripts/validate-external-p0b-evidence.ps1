...
try {
    ...
    $expectedEvidenceName = 'p0b-v1-' + [string]$receipt.receipt_id + '.json'
...
$expectedTests = [ordered]@{
    F01 = 'test_desktop_router.DesktopRouterQueueTests.test_exclusive_claim_publication_keeps_canonical_pending;test_desktop_router.DesktopRouterQueueTests.test_identical_producer_overlap_cannot_republish_claimed_request'
    F02 = 'test_desktop_router.DesktopRouterQueueTests.test_legacy_unfenced_claim_is_terminalized_as_uncertain'
    F03 = 'test_desktop_router.DesktopRouterQueueTests.test_receipt_payload_without_marker_is_authoritative_and_not_replayed;test_desktop_router.DesktopRouterQueueTests.test_receipt_payload_survives_marker_descriptor_close_failure'
    F04 = 'test_desktop_router.DesktopRouterQueueTests.test_orphan_terminal_receipt_recovers_as_unknown_and_survives_cleanup'
    F05 = 'test_desktop_router.DesktopRouterQueueTests.test_concurrent_terminal_finalizers_preserve_first_receipt'
    F06 = 'test_desktop_router.DesktopRouterQueueTests.test_wake_database_lock_preserves_pending_and_reconciles_once'
    F07 = 'test_desktop_router.DesktopRouterQueueTests.test_concurrent_conflicting_producers_publish_one_fingerprint'
    F08 = 'test_desktop_router.DesktopRouterQueueTests.test_explicit_safe_failure_advances_one_retry_generation;test_desktop_router.DesktopRouterQueueTests.test_retry_generation_ancestry_survives_response_cleanup;test_desktop_router.DesktopRouterQueueTests.test_stale_read_only_claim_advances_retry_generation;test_desktop_router.DesktopRouterQueueTests.test_mutating_claim_keeps_long_ttl_when_read_claim_would_expire'
    F09 = 'test_desktop_router.DesktopRouterQueueTests.test_target_lifecycle_failure_never_advances_retry_generation;test_desktop_router.DesktopRouterQueueTests.test_retry_generation_requires_explicit_json_booleans'
    F10 = 'test_runtime.PendingProjectMarkerTests.test_fresh_project_marker_precedes_unknown_create_and_same_event_recovers;test_runtime.PendingProjectMarkerTests.test_same_event_resumes_exact_pending_project_marker'
    F11 = 'test_runtime.PendingProjectMarkerTests.test_different_event_cannot_overwrite_a_pending_project_marker'
    ...
    if ([System.IO.Path]::GetFileName($workDirectory) -cne ('p0b-work-' + [string]$receipt.receipt_id)) {
    ...
    $mutexEvidence = $receipt.listener_stopped_receipt.lifecycle_mutex
    ...
    $preStatusArgv = @($receipt.listener_stopped_receipt.pre.status_argv)
    ...
    if ([int]$structured.schema_version -ne 1 -or
        ...
        $successfulIds.Count + @($structured.skipped_test_ids).Count -ne [int]$structured.tests_run) {
    ...
    foreach ($label in @('pre', 'post', 'final')) {
        $observation = $receipt.listener_stopped_receipt.$label
        $statusArgv = @($observation.status_argv)
        if ($statusArgv.Count -ne 12 -or
            ...
            -not (Get-NormalizedFullPath -Path ([string]$statusArgv[11])).Equals(
                $project, [System.StringComparison]::OrdinalIgnoreCase)) {
            throw "Receipt $label status argv differs from the exact stopped observation command."
        }
        $statusPath = Resolve-ExistingPath -Path `
            (Join-Path $workDirectory ("status-{0}.stdout.txt" -f $label)) `
            -Role ("retained {0} status capture" -f $label) -Leaf
        ...
        $statusBytes = Read-PinnedStreamBytes -Stream $statusStream `
            -MaximumBytes $script:MaximumCaptureBytes
        if ((Get-BytesSha256 -Bytes $statusBytes) -cne [string]$observation.status_stdout_sha256) {
            throw "Receipt $label status capture differs from its recorded hash."
        }
        $statusText = $script:StrictUtf8.GetString($statusBytes)
        $runtimeLines = @([regex]::Matches($statusText, '(?m)^Runtime:\s*([^\r\n]+)\s*$'))
        if ($runtimeLines.Count -ne 1 -or $runtimeLines[0].Groups[1].Value.Trim() -cne 'stopped') {
            throw "Receipt $label status capture does not report one exact stopped runtime."
        }
    }

    $preTime = [DateTimeOffset]::Parse([string]$receipt.listener_stopped_receipt.pre.observed_at_utc)
    ...
    $postTime = [DateTimeOffset]::Parse([string]$receipt.listener_stopped_receipt.post.observed_at_utc)
    $finalTime = [DateTimeOffset]::Parse([string]$receipt.listener_stopped_receipt.final.observed_at_utc)
