...
if (-not $identity.Verified) {
    throw "Listener PID $bridgePid exists, but its Python command line could not be verified; refusing to stop any process."
}
...
while ((Get-Date) -lt $deadline) {
    ...
    if (-not $identity.Verified) {
        # Win32_Process may lose the command line while the verified Listener is
    ...
    if (-not $identity.IsBridge) {
        Remove-Item -LiteralPath $pidFile -Force -ErrorAction SilentlyContinue
        Write-Output "Listener exited; reused PID $bridgePid now belongs to $($identity.ProcessName). No process was stopped."
        exit 0
    }
    Start-Sleep -Milliseconds 500
}
...
if ($identity.Exists -and -not $identity.Verified) {
    throw "Listener PID $bridgePid changed to an unverifiable Python process; refusing to force-stop it."
}
if ($identity.Exists -and $identity.IsBridge) {
    ...
    Write-Output "Listener exited; reused PID $bridgePid belongs to $($identity.ProcessName). No process was stopped."
}
Remove-Item -LiteralPath $pidFile -Force -ErrorAction SilentlyContinue
} finally {
