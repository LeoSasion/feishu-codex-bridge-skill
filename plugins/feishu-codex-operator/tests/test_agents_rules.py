...
class AgentsRulesTests(unittest.TestCase):
    ...
    def test_rules_admit_only_minimal_beeper_relay_and_read_only_catalog(self) -> None:
        ...
        self.assertIn("`gpt-5.6-luna` and `low` reasoning when", rules)
        self.assertIn("deterministic local `beeper`, Spark, or Luna", rules)
        self.assertIn("catalog uses `visibility: list`", rules)
        self.assertIn("fixed approved identity declaration", rules)
        self.assertIn("without model weights or sampling", rules)
        self.assertIn("uncertain local outcome is terminal and never falls back", rules)
        self.assertIn("Spark with `low`\n  reasoning is forbidden in normal selection", rules)
        ...
        self.assertIn("exactly one same-event\n  Luna/low queue attempt", rules)
