...
TEST_TEMP_ROOT = Path(os.environ.get("FEISHU_OPERATOR_TEST_TMP", tempfile.gettempdir()))
if "FEISHU_OPERATOR_TEST_TMP" not in os.environ:
    TEST_TEMP_ROOT = TEST_TEMP_ROOT / "feishu-codex-operator-tests"
TEST_TEMP_ROOT.mkdir(parents=True, exist_ok=True)
sys.path.insert(0, str(SKILL_ROOT / "scripts"))

from operator_core.config import load_config  # noqa: E402
from operator_core.lark import (  # noqa: E402
    LarkEventConsumer,
    ReplyResult,
...
class RoutingTests(unittest.TestCase):
    def test_event_consumer_stderr_never_logs_remote_message_or_scope_text(self) -> None:
        ...
        with self.assertLogs("feishu-codex-operator", level="INFO") as records:
    ...
    def test_plain_text_single_part_preserves_all_whitespace_and_unicode(self) -> None:
        ...
        with patch("operator_core.lark.run_command", return_value=success) as command:
    ...
    def test_markdown_retry_reuses_the_frozen_post_wire_payload(self) -> None:
        ...
        with (
            patch(
                "operator_core.lark._markdown_post_content",
                side_effect=AssertionError("retry must not re-render markdown"),
            ),
            patch("operator_core.lark.run_command", return_value=success) as command,
        ):
            result = reply_to_message(
                "lark-cli",
                event,
    ...
    def test_withdrawn_message_is_a_terminal_reply_failure(self) -> None:
        ...
        with patch("operator_core.lark.run_command", return_value=withdrawn):
        ...
        with patch("operator_core.lark.run_command", return_value=unknown):
            result = reply_to_message("lark-cli", event, "answer", config)
        self.assertFalse(result)
    ...
    def test_rate_limit_and_network_failures_remain_retryable(self) -> None:
        ...
        for label, failure in failures.items():
            with self.subTest(label=label):
                with patch("operator_core.lark.run_command", return_value=failure):
                    result = reply_to_message("lark-cli", event, "answer", config)
                self.assertIsInstance(result, ReplyResult)
                self.assertFalse(result)
                self.assertTrue(result.retryable)
    ...
    def test_released_lease_remains_viable_while_host_process_is_alive(self) -> None:
        with tempfile.TemporaryDirectory(dir=TEST_TEMP_ROOT) as temporary:
            ...
            config = replace(
                load_config(),
                project_root=root,
                runtime_dir=root / ".codex" / "feishu-codex-operator-runtime",
