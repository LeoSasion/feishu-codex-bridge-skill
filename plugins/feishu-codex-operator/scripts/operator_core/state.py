...
# State tombstone only: never adopt retired task ownership as a current route.
...
OUTBOUND_ENVELOPE_DOMAIN = b"feishu-codex-operator/outbound-envelope/v1\0"
...
class DurableState:
    ...
    def __init__(self, path: Path) -> None:
        ...
        with self._lock:
            ...
            self._connection.execute(
                ...
                (INTERRUPTED_REPLY, "operator restarted after model turn started", now),
            )
            self._connection.execute(
    ...
    def mark_responder_dispatched(self, event_id: str, thread_id: str) -> None:
        with self._lock:
            self._connection.execute(
                """
                UPDATE inbox_events
                ...
                """,
                (thread_id, None, time.time(), event_id),
            )
            self._connection.commit()

    def mark_responder_not_started(self, event_id: str) -> None:
        """Clear a provisional relay dispatch after a proven pre-delivery failure."""
    ...
    def admit_control(self, event_id: str) -> bool:
        """Consume control admission before any wizard or binding action."""
            ...
            try:
                row = self._connection.execute(
                    ...
                    WHERE event_id=? AND status='reply_pending'
                      ...
                      AND last_error='operator restarted after model turn started'
                    """,
    ...
    def verified_outbound(
        ...
        if "_operator_scope" in event:
            event_scope = event.get("_operator_scope")
...
class AccessPolicy:
    ...
    def decide(self, *, sender_open_id: str, chat_id: str, chat_type: str) -> AccessDecision:
        ...
        return AccessDecision(False, "denied", "not on the operator allowlist")
...
class SessionStore:
    ...
    def _load(self) -> dict[str, dict[str, Any]]:
        ...
        if (
            payload.get("schema_version") in {4, 5}
            and payload.get("session_owner") in {RETIRED_SESSION_OWNER, "beeper"}
        ):
        ...
        if (
            ...
            # changing only their transport owner. The on-demand read-only
            # catalog resolves current identity when the user runs /init.
            migrated_sessions: dict[str, dict[str, Any]] = {}
            for scope, value in sessions.items():
                data = dict(value)
                data["session_owner"] = self.SESSION_OWNER
    ...
    def canonical_scope(scope: str) -> str:
        """Remove only the final access-policy suffix from a operator scope."""
