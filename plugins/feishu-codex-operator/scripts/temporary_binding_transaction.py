...
read-only task tools.  Mutations require a stopped Operator and are protected by
the same operator lock filename used by the Operator.
"""
...
from operator_core.runtime import is_process_running
...
class MaintenanceLock:
    """Fail closed unless the normal Operator has released its runtime lock."""

    def __init__(self, runtime_dir: Path) -> None:
        self.runtime_dir = runtime_dir
        self.lock_path = runtime_dir / "operator.lock"
        self.pid_path = runtime_dir / "operator.pid"
        self.acquired = False
    ...
    def __enter__(self) -> "MaintenanceLock":
        ...
        if health_status and health_status != "stopped":
            raise ValueError("Operator health is not stopped")
        if self.pid_path.exists():
            try:
                pid = int(self.pid_path.read_text(encoding="utf-8").strip())
            except (OSError, ValueError):
                raise ValueError("Operator pid reference is present but invalid") from None
            if is_process_running(pid):
                raise ValueError("Operator pid is still active")
            raise ValueError("stale Operator pid reference must be repaired before maintenance")
        try:
            descriptor = os.open(self.lock_path, os.O_CREAT | os.O_EXCL | os.O_WRONLY)
        except FileExistsError:
            raise ValueError("Operator or another maintenance action still owns the operator lock") from None
        try:
...
def _runtime_dir(value: str) -> Path:
    ...
    if not candidate.is_dir():
        raise ValueError("operator runtime directory does not exist")
    return candidate
...
def bind(args: argparse.Namespace) -> dict[str, Any]:
    if not args.operator_stopped_acknowledged:
        raise ValueError("binding requires an explicit stopped-Operator acknowledgement")
...
def rollback(args: argparse.Namespace) -> dict[str, Any]:
    if not args.operator_stopped_acknowledged:
        raise ValueError("rollback requires an explicit stopped-Operator acknowledgement")
    runtime_dir = _runtime_dir(args.runtime_dir)
    with MaintenanceLock(runtime_dir):
        store = SessionStore(runtime_dir / "sessions.json")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Reversible Feishu operator diagnostic binding")
    ...
    bind_parser.add_argument("--operator-stopped-acknowledged", action="store_true")

    rollback_parser = subcommands.add_parser("rollback")
    rollback_parser.add_argument("--transaction-id", required=True)
    rollback_parser.add_argument("--operator-stopped-acknowledged", action="store_true")
