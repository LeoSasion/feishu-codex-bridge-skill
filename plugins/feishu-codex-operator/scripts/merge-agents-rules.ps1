...
$fragmentPath = Join-Path $skillRoot 'assets\AGENTS.feishu-codex-operator.md'
$agentsPath = Join-Path $resolvedProjectRoot 'AGENTS.md'
$backupRoot = Join-Path $resolvedProjectRoot (
    '.codex\feishu-codex-operator-runtime\backups\' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '-agents-rules'
)
...
if ($startMatches.Count -ne $endMatches.Count -or $startMatches.Count -gt 1) {
    throw ('AGENTS.md has malformed or duplicate Feishu operator markers. ' +
...
if ($startMatches.Count -eq 0) {
    ...
    if ($endIndex -le $startIndex) {
        throw 'AGENTS.md Feishu operator markers are out of order. No changes were written.'
    }
...
if ($status -eq 'missing') {
    ...
    Write-Output "Updated only the Feishu operator managed rules in $agentsPath"
}
