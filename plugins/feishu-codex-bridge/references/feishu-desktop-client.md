...
`-DesktopInstallConsent` records that the current request includes this exact
install outcome; it is not a reason to invoke an external authorization relay or ask for a ritual
approval phrase.
...
unrelated chats into Bridge logs, responder prompts, tests, or Skill files.
...
visible reply and Bridge logs. Otherwise stop before the representational
