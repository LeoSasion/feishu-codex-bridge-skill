...
## Recommended profile: `openclaw-common-chat`
...
Describe that breadth before starting the flow and treat the Feishu
authorization page as the authoritative list the user personally reviews.
Granting a scope never widens the current user request to a later operation by
itself.
...
If the Desktop sandbox rejects that exact directory, use the supported
exact-path access mechanism only for the per-run QR directory and retry
generation once with the same URL. Do not start a generic consent loop or
restart the authorization merely to obtain a writable output path.
...
Do not classify that pattern yet. Use the supported credential-visible process
only to read the current Windows credential entries and repeat `lark-cli auth
status --json --verify` once. If that result is `ready`/`valid`, do
...
expired, open exactly one new official QR flow when restoring this setup is in
the current request. Run `lark-cli config init --new`, let the user complete the
Feishu-owned QR page, then run the normal `--recommend` user OAuth flow. Do not
ask for a separate generic consent phrase or invoke an external authorization relay. The official re-init may recover the
...
denied` for the system temp directory, use the supported access mechanism only
for that exact per-run directory and retry QR generation once with the same
opaque URL. Do not restart config-init or OAuth. Windows may expose the same temp root once as an
...
CLI's user-side IM domain; it would not declare or grant Bot tenant scopes. Do
not add it automatically or present it as a Bot-permission repair. Granting user
scopes also does not widen the current request to later message,
chat-administration, recall, urgent-delivery, member, moderation, or other
high-impact actions. When one of those actions is explicitly requested, execute
its ordinary steps without an extra generic confirmation loop.

The Bridge itself runs as the **Bot**. Bot tenant scopes are distinct
...
In particular, the default runtime remains locked to allowlisted users/groups and
...
relevant tenant permission. `im:message.group_msg` is a Bot capability requiring
Feishu administrator grant, not evidence from user OAuth.
...
`auth login` as a Bot-scope repair, do not call a platform administrator-grant API
...
path as a separate setup outcome that the user must explicitly request.

Permissions do not widen the current request to access-policy changes,
event-subscription changes, app publication, Bridge mounting, or any producer
creation or activation. When the current request includes ordinary Bridge setup
outcomes, perform only the Bridge, access, manifest, PID, plugin, Hook, and
zero-rule-tombstone transactions in scope. Every retired producer surface remains
