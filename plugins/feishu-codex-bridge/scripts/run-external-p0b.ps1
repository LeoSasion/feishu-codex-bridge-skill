...
if (-not $ExternalTestRunnerAcknowledged -or $env:FEISHU_BRIDGE_EXTERNAL_TEST_RUNNER -ne '1') {
    throw ('P0-B requires the verified external-supervisor environment. Set ' +
...
function Test-AdmissibleStoppedStatus {
    ...
    $presentIsBoolean = $manifest.present -is [bool]
    $validIsBoolean = $manifest.valid -is [bool]

    if ($statusCode -ceq 'pass') {
        return (
            $presentIsBoolean -and [bool]$manifest.present -and
            $validIsBoolean -and [bool]$manifest.valid -and
    ...
    return (
        $presentIsBoolean -and -not [bool]$manifest.present -and
        $validIsBoolean -and -not [bool]$manifest.valid -and
...
function Get-BridgeObservation {
    param(
        ...
        [Parameter(Mandatory = $true)][string]$PythonExecutable,
        [Parameter(Mandatory = $true)][string]$BeeperQueueScript,
        [Parameter(Mandatory = $true)][string]$Project,
        [Parameter(Mandatory = $true)][string]$Runtime,
    ...
    $statusLines = @($capture.Stdout -split '\r?\n' | Where-Object {
        -not [string]::IsNullOrWhiteSpace($_)
    })
    if ($statusLines.Count -ne 1) {
        throw "Bridge status $Label did not return one JSON object."
    }
    try {
        $status = $statusLines[0] | Microsoft.PowerShell.Utility\ConvertFrom-Json -ErrorAction Stop
    } catch {
        throw "Bridge status $Label did not return valid JSON."
    }
    $health = $status.health_snapshot
    $queueCounts = if ($null -eq $health) { $null } else { $health.queue_counts }
    $queueCountNames = if ($null -eq $queueCounts) {
        @()
    } else {
        @($queueCounts.PSObject.Properties.Name | Sort-Object)
    }
    $expectedQueueCountNames = @(
        'completed', 'control_sending', 'queued', 'reply_pending',
        'retryable_failed', 'running', 'terminal_failed'
    )
    $queueCountsAreExact = (
        ($queueCountNames -join "`n") -ceq ($expectedQueueCountNames -join "`n")
    )
    $schemaCurrentIsBoolean = $health.schema_current -is [bool]
    $actionableRetryIsZero = (
        ($health.actionable_retryable_failed -is [int] -or
         $health.actionable_retryable_failed -is [long]) -and
        [long]$health.actionable_retryable_failed -eq 0
    )
    ...
    $processIdentityIsStopped = (
        $health.process_identity_current -is [bool] -and
        -not [bool]$health.process_identity_current
    )
    $healthSchemaIsAdmissible = (
        $schemaCurrentIsBoolean -and
        (([bool]$health.schema_current -and $actionableRetryIsZero) -or
         (-not [bool]$health.schema_current -and
          $null -eq $health.actionable_retryable_failed))
    )
    if ([int]$status.schema_version -ne 1 -or
        ...
        [bool]$status.runtime.running -or
        [bool]$health.present -ne $true -or
        [bool]$health.valid -ne $true -or
        [string]$health.status -cne 'stopped' -or
        [bool]$health.event_consumer -ne $false -or
        [int]$health.active_turns -ne 0 -or
        -not $processIdentityIsStopped -or
        -not $queueCountsAreExact -or
        -not $healthSchemaIsAdmissible -or
        [bool]$health.dial_inflight -ne $false -or
        [int]$queueCounts.queued -ne 0 -or
        [int]$queueCounts.running -ne 0 -or
        [int]$queueCounts.control_sending -ne 0 -or
        [int]$queueCounts.reply_pending -ne 0) {
        throw "Bridge status $Label did not report the exact stopped and idle runtime."
    }

    $beeperArguments = @(
        '-S', '-B', $BeeperQueueScript,
        '--runtime-dir', $Runtime,
        '--queue-namespace', 'beeper',
        'status'
    )
    $beeperCapture = Invoke-CapturedProcess -Executable $PythonExecutable `
        -Arguments $beeperArguments -WorkingDirectory $Project -EnvironmentOverrides @{} `
        -CapturePrefix (Join-Path $WorkDirectory ("beeper-status-{0}" -f $Label))
    if ($beeperCapture.ExitCode -ne 0 -or
        -not [string]::IsNullOrWhiteSpace([string]$beeperCapture.Stderr)) {
        throw "Beeper queue status $Label failed or produced stderr."
    }
    $beeperLines = @($beeperCapture.Stdout -split '\r?\n' | Where-Object {
        -not [string]::IsNullOrWhiteSpace($_)
    })
    if ($beeperLines.Count -ne 1) {
        throw "Beeper queue status $Label did not return one JSON object."
    }
    try {
        $beeperStatus = $beeperLines[0] |
            Microsoft.PowerShell.Utility\ConvertFrom-Json -ErrorAction Stop
    } catch {
        throw "Beeper queue status $Label did not return valid JSON."
    }
    if ([bool]$beeperStatus.ok -ne $true -or
        [int]$beeperStatus.pending -ne 0 -or
        [int]$beeperStatus.claimed -ne 0 -or
        [bool]$beeperStatus.dial_inflight -ne $false) {
        throw "Beeper queue status $Label did not report the exact idle queue."
    }
    ...
    return [pscustomobject][ordered]@{
        ...
        status_stderr_sha256 = $capture.StderrSha256
        health_idle = $true
        beeper_status_argv = @($beeperCapture.Argv)
        beeper_status_exit_code = 0
        beeper_status_stdout_sha256 = $beeperCapture.StdoutSha256
        beeper_status_stderr_sha256 = $beeperCapture.StderrSha256
        beeper_queue_idle = $true
...
function Copy-AuditedDesktopSnapshot {
    ...
    foreach ($file in @($component.files)) {
        ...
        if ($destinationHash -cne [string]$file.sha256 -or
            ...
            throw 'External Desktop snapshot destination bytes differ from the audited source.'
        }
    ...
    if ($actualCanonical -cne $expectedCanonical) {
        throw 'External Desktop snapshot destination manifest differs from the audited Desktop component.'
    }
    return [pscustomobject]@{
        Root = $Destination
...
$beeperQueueScript = Resolve-ExistingPath -Path `
    (Join-Path $sourceSnapshot.Root 'scripts\beeper_queue_cli.py') `
    -Role 'audited snapshot Beeper queue status helper' -Leaf
...
$preObservation = Get-BridgeObservation -Label 'pre' -ShellExecutable $shell `
    -Dispatcher $dispatcher -PythonExecutable $python -BeeperQueueScript $beeperQueueScript `
...
$beeperTest = 'test_beeper_queue.BeeperQueueTests.'
$clientTest = 'test_beeper_client.BeeperClientContractTests.'
$envEntrypointTest = 'test_agents_rules.BridgeEnvEntrypointTests.'
$faultTests = [ordered]@{
    F01 = @(
        "${beeperTest}test_namespace_and_registration_are_closed_and_immutable"
        "${beeperTest}test_beeper_and_tombstones_cannot_be_business_responders"
    )
    F02 = @(
        "${clientTest}test_argv_contains_only_fixed_control_and_opaque_page"
        "${clientTest}test_same_request_never_spawns_twice"
    )
    F03 = @("${clientTest}test_reserved_beeper_loads_exact_uri_once_without_requeue")
    F04 = @("${clientTest}test_load_assist_failure_is_safe_and_terminal")
    F05 = @(
        "${clientTest}test_readonly_unknown_is_safe_terminal_and_not_retried"
        "${beeperTest}test_readonly_claim_expiry_is_terminal_and_not_replayed"
    )
    F06 = @("${beeperTest}test_unclaimed_failure_cas_and_claim_are_exclusive")
    F07 = @("${beeperTest}test_finish_waits_for_delayed_beeper_claim")
    F08 = @(
        "${beeperTest}test_final_callback_finish_is_exactly_once"
        "${clientTest}test_completed_send_requires_top_level_final_callback_source"
    )
    F09 = @(
        "${beeperTest}test_final_callback_conflict_fails_closed_and_scrubs_capability"
        "${beeperTest}test_catalog_tamper_is_rejected_and_scrubbed"
    )
    F10 = @(
        "${beeperTest}test_catalog_interrupted_consume_is_not_replayed_and_ages_out"
        "${clientTest}test_final_callback_timeout_is_terminal_and_not_retried"
    )
    F11 = @(
        "test_runtime.StableConversationScopeTests.test_binding_commit_control_crash_is_terminal_after_reopen"
        "${beeperTest}test_catalog_is_staged_answer_free_then_consumed_once"
    )
...
try {
    ...
    $postObservation = Get-BridgeObservation -Label 'post' -ShellExecutable $shell `
        -Dispatcher $dispatcher -PythonExecutable $python -BeeperQueueScript $beeperQueueScript `
        -Project $project -Runtime $runtime -WorkDirectory $workDirectory
...
if (($actualStructuredProperties -join "`n") -cne ($expectedStructuredProperties -join "`n") -or
    ...
    @($structuredTestResult.skipped_test_ids).Count -ne 0 -or
...
if ($skipped -ne 0 -or $successfulTestIds.Count -ne $testsRun) {
...
$finalObservation = Get-BridgeObservation -Label 'final' -ShellExecutable $shell `
    -Dispatcher $dispatcher -PythonExecutable $python -BeeperQueueScript $beeperQueueScript `
    -Project $project -Runtime $runtime -WorkDirectory $workDirectory
...
$receipt = [pscustomobject][ordered]@{
    ...
    bridge_stopped_receipt = [pscustomobject][ordered]@{
        pre = $preObservation
...
$receiptName = "p0b-v2-{0}.json" -f $receiptId
