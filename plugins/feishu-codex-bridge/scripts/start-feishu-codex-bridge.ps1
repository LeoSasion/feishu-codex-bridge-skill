...
function Start-DetachedLaunchHelper {
    ...
    if (-not (Test-Path -LiteralPath $windowsPowerShell -PathType Leaf)) {
        throw 'Windows PowerShell is unavailable for detached Bridge launch.'
    }
    ...
    if (-not $result -or [int]$result.ReturnValue -ne 0 -or [int]$result.ProcessId -le 0) {
        ...
        throw "Detached Bridge helper creation failed with WMI return value $returnValue."
    }
    ...
    if ([int]::TryParse($existingPidText, [ref]$existingPid)) {
        ...
        if ($identity.Exists) {
            if (-not $identity.Verified) {
                throw "Bridge PID $existingPid exists, but its Python command line could not be verified; refusing to start a second runtime."
            }
...
if (-not $DetachedLaunch) {
    ...
    if ([string]$currentLaunchLease.launch_status -ceq 'reserved') {
        ...
        Set-LeaseLaunchState -ExactLeaseId $LeaseId -LaunchState failed -Release
        throw 'Bridge lease is not in a valid detached-launch state.'
    }

    # The helper must acquire this same mutex before it can revalidate the
    # lease and launch Python. Release it before waiting for the exact Bridge.
    ...
    while ((Get-Date) -lt $launchDeadline) {
        ...
        if (-not $launchLease -or [string]$launchLease.status -cne 'active' -or
            ...
            throw "Detached Bridge helper $helperPid did not retain an active launch lease."
        }
    ...
    throw "Detached Bridge helper $helperPid did not produce a verified Bridge before its bounded deadline."
}
...
$startedIdentity = Get-BridgeProcessIdentity -ProcessId $process.Id -BridgeScript $bridgeScript
if (-not $startedIdentity.Exists -or -not $startedIdentity.Verified -or -not $startedIdentity.IsBridge) {
