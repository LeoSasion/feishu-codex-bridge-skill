...
foreach ($hookPair in $hookPairs) {
    ...
    if ($sourceHash -cne $installedHash) {
        throw (
            ...
            "P0-B did not start. Do not copy it manually; run the maintained " +
            "Bridge hook refresh transaction, verify it, then rerun."
        )
    }
}
...
$runRoot = Join-Path $artifact "p0b-$runTag"
if (Test-Path -LiteralPath $runRoot) {
    throw "Unique P0-B artifact run destination already exists: $runRoot"
}
...
if (-not $artifactVerified.Equals($artifact, [System.StringComparison]::OrdinalIgnoreCase)) {
    throw 'ArtifactRoot canonical path changed before P0-B run-directory creation.'
}
New-Item -ItemType Directory -Path $runRoot -ErrorAction Stop | Out-Null
$workRoot = Join-Path $runRoot 'work'
$evidenceRoot = Join-Path $runRoot 'evidence'
Assert-NoReparseExistingPathPrefix -Path $runRoot -Role 'P0-B run root'
...
if (-not (Test-IsWithinPath -Root $artifactRecord.DevicePath -Candidate $runRecord.DevicePath) -or
    ...
    throw 'P0-B run root escaped the physical ArtifactRoot boundary.'
}
...
function Invoke-CleanPowerShellJson {
    ...
    if (-not [string]::IsNullOrWhiteSpace($stderr)) {
        throw "$StageName emitted unexpected stderr on a zero exit."
    }
    $nonempty = @(
        ($stdout -split "`r?`n") |
            Where-Object { -not [string]::IsNullOrWhiteSpace($_) }
...
try {
    ...
    if ([int]$envelope.schema_version -ne 2 -or
        ...
        [string]$envelope.evidence_file -cnotmatch '^p0b-v2-[a-f0-9-]{36}\.json$' -or
        ...
        throw 'P0-B supervisor JSON failed its envelope contract.'
    }
    ...
    if ([int]$validation.validation_schema_version -ne 2 -or
        ...
        throw 'P0-B semantic validator JSON failed its wrapper contract.'
    }
}
finally {
    [Environment]::SetEnvironmentVariable(
