...
first use, `bridge preflight`, permission work, mounting, or headless Bridge
operation. The Bridge runtime and Feishu CLI do not require the desktop client.
...
impact, and that the user must complete authentication. Obtain explicit current
consent; then run:
...
If the app-control launch approval times out, do not loop or claim the client
failed to start. Under the same already-disclosed one-launch consent, start the
exact verified executable once through the platform launcher, then re-list
windows. If screenshot capture fails with `SetIsBorderRequired` or another
...
user-requested test target. Do not persist, summarize, quote, or inject
unrelated chats into Bridge logs, target prompts, tests, or Skill files.
...
the exact command for the user to send manually, and continue Bridge-side
...
that composer is still unique. This refresh does not authorize typing or
sending. Stop on a second stale-state error or any geometry failure.

Do not automate an authentication dialog, credentials, CAPTCHA, security
prompt, privacy permission, account choice, or QR approval. The user must
complete authentication. After the user reports completion, refresh the same
window and reclassify the state.

Installing or logging in is not consent to send a message. Obtain fresh exact
approval before a live conversation test, keep it within a dedicated test
contact/group, and verify both the visible reply and Bridge logs.
...
- If the exact official metadata request is blocked by the sandbox, request
  network access for that request only. Do not switch to a mirror.
...
- Installing, logging in, or observing the client never authorizes sending. A
  live send remains a fresh representational-action approval.
